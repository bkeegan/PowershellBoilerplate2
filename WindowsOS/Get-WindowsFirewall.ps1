...
function Get-WindowsFirewall
{
	...
	Process
	{
		$finalResult = Invoke-Command -Computer $computer -ScriptBlock {
			...
			If($args[1].IsPresent -eq $true)
			{
				...
				for($i=0;$i -le $queryResult.GetUpperBound(0);$i++)
				{
					...
					if($matches.count -eq 3) 
					{
						$propertyName = $matches[1] -replace "\s+",""
						Add-Member -InputObject $firewallDetails -Type NoteProperty -Name "Domain_$propertyName" -value $matches[2]
					}
